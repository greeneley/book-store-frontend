...
export const Cart: React.FC = () => {
	...
	const increaseItem = useCallback(
		(item: any) => {
			CartItemService.updateQuantityItem(item.quantity + 1, item.book_id).then(() => navigate("."));
		},
		[navigate]
	);
	...
	const columns = [
		...
		},
		{
			...
			render: (quantity: number, item: CartInfo) => {
				return (
					<span>
						<MinusCircleOutlined className="mr-8" onClick={() => decreaseItem(item)} />
						{quantity}
						<PlusCircleFilled className="ml-8" onClick={() => increaseItem(item)} />
					</span>
				);
			}
		},
			...
			render: (_: any, record: any) => {
				return dataSource.length >= 1 ? (
					<Popconfirm
						...
						onConfirm={async () => {
							CartItemService.removeCartItem(record.key).then(() => navigate("."));
						}}>
						<a>Xóa</a>
					</Popconfirm>
				) : null;
		...
		<>
			<div className="my-10 p-8">
				...
				<Row>
					...
					<Col span={8}>
						<Title level={3}>Tổng số sản phẩm: {cartInfo.cart_items.length}</Title>
						<Title level={4}>Tổng giá trị đơn hàng: {convertToCurrency(cartInfo.total)}</Title>
